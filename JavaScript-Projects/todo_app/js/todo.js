...
function show() {
    ...
    for (var i = 0; i < todos.length; i++) {
        /*this also displays the task as a list and creates tghe button with the "x"*/
...
/* Create function to remove a to do item from the array */
function remove() {
    var item = this.getAttribute('item');
    var todos = get_todos();
    todos.splice(item, 1);
    localStorage.setItem('todo', JSON.stringify(todos));
    /* Look into show() to see how to display a removed item on screen */
    show();
